...
class MyArray<T> implements IMyArray<T> {
  ...
  forEach(
    callback: (value: T, index: number, array: MyArray<T>) => void,
    thisArg?: T
  ): void {
  ...
  map<U>(
    callback: (value: T, index: number, array: MyArray<T>) => U,
    thisArg?: any[]
  ): MyArray<U> {
  ...
  reduce(
    ...
    if (this.length === 0 && initialValue === undefined) {
      throw new TypeError(
        "Array is empty, InitialValue is specified, callback should not  be called"
      );
    }
